...
class MaxNormConstraint:
    """Max-norm constraint for weights (matches braindecode parametrization)."""
    
    def __init__(self, max_norm=1.0, dim=0):
        self.max_norm = max_norm
        self.dim = dim
    
    def __call__(self, module, input):
        """Apply max-norm constraint before forward pass."""
        if hasattr(module, 'weight') and module.weight is not None:
            with torch.no_grad():
                norm = module.weight.norm(2, dim=self.dim, keepdim=True)
                desired = torch.clamp(norm, 0, self.max_norm)
                module.weight *= (desired / (norm + 1e-8))


class Conv2dWithConstraint(nn.Conv2d):
    """Conv2d with max-norm constraint (matches ParametrizedConv2dWithConstraint)."""
    
    def __init__(self, *args, max_norm=1.0, **kwargs):
        super().__init__(*args, **kwargs)
        self.max_norm = max_norm
        self.register_forward_pre_hook(MaxNormConstraint(max_norm, dim=0))


class LinearWithConstraint(nn.Linear):
    """Linear layer with max-norm constraint (matches ParametrizedLinearWithConstraint)."""
...
class EEGNeX_Standalone(nn.Module):
    ...
    def __init__(
        ...
        # Block 3: Spatial depthwise conv + BN + ELU + AvgPool + Dropout
        self.block_3 = nn.Sequential(
            Conv2dWithConstraint(
                filter_2, filter_3, kernel_size=(n_chans, 1),
                groups=filter_2, max_norm=max_norm_conv, bias=False, padding=0
            ),
            nn.BatchNorm2d(filter_3),
            nn.ELU(),
            nn.AvgPool2d((1, avg_pool_block4), padding=(0, 1)),
            nn.Dropout(drop_prob),
        )
        
        # Block 4: Conv2d + BatchNorm (NO ELU/pooling/dropout!)
        ...
        # Block 5: Conv2d + BN + ELU + AvgPool + Dropout + Flatten
        self.block_5 = nn.Sequential(
            nn.Conv2d(filter_2, filter_1, kernel_size=(1, kernel_block_5),
                     dilation=(1, dilation_block_5), padding='same', bias=False),
            nn.BatchNorm2d(filter_1),
            nn.ELU(),
            nn.AvgPool2d((1, avg_pool_block5), padding=(0, 1)),
            nn.Dropout(drop_prob),
            nn.Flatten(),
        )
        
        # Calculate final size after all operations
        # After block_3: time -> (time + 2) // 4 with padding (0,1)
        # After block_5: time -> (time + 2) // 8 with padding (0,1)
        # Shape tracking: 200 -> 50 (block_3) -> 50 (block_4) -> 6 (block_5)
        # Final: 8 channels * 6 time = 48 features
        final_features = 48  # Empirically determined from braindecode
...
class Submission:
    ...
    def predict_challenge_2(self, X: np.ndarray) -> np.ndarray:
        ...
        # Rearrange to 4D happens inside the model
