#!/usr/bin/env python3
"""
Challenge 1: Improved Training (Fast Version)
Fixed: Load data once, then split (not load twice)
"""
...
print("=" * 80)
print("🚀 CHALLENGE 1: IMPROVED TRAINING (FAST) → Target Pearson r ≥ 0.91")
print("=" * 80)
...
# ============================================================================
# DATA LOADER (load ONCE, augment on-the-fly)
# ============================================================================

class ResponseTimeDataset(Dataset):
    """Load EEG windows once, augment during training"""
...
# ============================================================================
# MAIN (FIXED: load once, split with indices)
# ============================================================================
...
def main():
    ...
    try:
        # Load data ONCE
        print("\n" + "="*80)
        full_dataset = ResponseTimeDataset(args.data_dirs, augment=False)
        ...
        train_dataset_aug = ResponseTimeDataset.__new__(ResponseTimeDataset)
        train_dataset_aug.segments = full_dataset.segments[train_indices]
        train_dataset_aug.response_times = full_dataset.response_times[train_indices]
        train_dataset_aug.subject_ids = full_dataset.subject_ids[train_indices]
        train_dataset_aug.augment = True
        
        # Create val dataset without augmentation
        val_dataset = ResponseTimeDataset.__new__(ResponseTimeDataset)
        val_dataset.segments = full_dataset.segments[val_indices]
        val_dataset.response_times = full_dataset.response_times[val_indices]
        val_dataset.subject_ids = full_dataset.subject_ids[val_indices]
        val_dataset.augment = False
        
        train_loader = DataLoader(train_dataset_aug, batch_size=args.batch_size, shuffle=True, num_workers=2)
        val_loader = DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False, num_workers=2)
        
        print(f"\n📊 Data Split:")
        print(f"   Train: {len(train_dataset_aug)} samples")
