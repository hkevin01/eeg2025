#!/usr/bin/env python3
"""
Challenge 2: Correct Training - Following Starter Kit
======================================================
Predicting externalizing factor (p_factor) from contrastChangeDetection task.

Key points from starter kit:
- Task: contrastChangeDetection (NOT resting!)
- Target: p_factor (externalizing)
- Windows: 4 seconds with 2-second stride, then random crop to 2 seconds
- Model: EEGNeX (generalization-focused architecture)
- Regularization: L1 loss, dropout, avoid overfitting
"""
import os
import sys
import time
...
print("="*80)
print("🎯 CHALLENGE 2: EXTERNALIZING PREDICTION (CORRECT VERSION)")
print("="*80)
print("Task: contrastChangeDetection (following starter kit)")
print("Target: p_factor (externalizing factor)")
print("Model: EEGNeX (generalization-focused)")
print("Strategy: Avoid overfitting, maximize generalization")
print("="*80)
print()
...
def train_model(model, train_loader, val_loader, device, epochs=20, lr=0.002):
    """Train with L1 loss and early stopping"""
    
    optimizer = optim.Adamax(params=model.parameters(), lr=lr)
