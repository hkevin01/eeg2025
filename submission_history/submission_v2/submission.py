...
class Submission:
    ...
    def get_model_challenge_1(self):
        ...
        """
        import sys
        model_challenge1 = ImprovedResponseTimeCNN().to(self.device)

        # Load trained weights
        try:
            # Try to load from checkpoints/response_time_improved.pth
            try:
                weights_path = resolve_path("response_time_improved.pth")
            except FileNotFoundError:
                try:
                    weights_path = resolve_path("checkpoints/response_time_improved.pth")
                except FileNotFoundError:
                    try:
