    ...
    def _compress_fft(self, data: np.ndarray) -> np.ndarray:
        ...
        if data.ndim == 1:
            ...
            for channel in range(data.shape[0]):
                ...
                compressed_coeffs = fft_coeffs * mask
                compressed.append(
                    np.concatenate([compressed_coeffs.real, compressed_coeffs.imag])
                )
...
class AdaptiveQuantizer(nn.Module):
    ...
    def __init__(
        self, num_bits: int = 8, signed: bool = True, learnable_scale: bool = True
    ):
        """
...
class CompressionAwareLoss(nn.Module):
    ...
    def forward(
        ...
        if features is not None:
            ...
            feature_metrics = self.compression_metrics.compute_feature_compression(
                features
            )
            ...
            compression_penalty = torch.relu(
                torch.tensor(self.target_compression_ratio)
...
def evaluate_model_compression(
    model: nn.Module, data_loader: torch.utils.data.DataLoader, device: str = "cuda"
) -> Dict[str, float]:
    """
    ...
    prediction_metrics = compression_metrics.compute_compression_metrics(
    ...
    results = {
        ...
        "prediction_compression_ratio": max(
            [m["compression_ratio"] for m in prediction_metrics.values()]
        ),
        "model_parameters": sum(p.numel() for p in model.parameters()),
        "model_size_mb": sum(p.numel() * 4 for p in model.parameters())
        / (1024**2),  # Assuming float32
    }
