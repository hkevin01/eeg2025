...
class StarterKitDataLoader:
    ...
    def __init__(
        ...
        try:
            logger.info(f"Initializing StarterKitDataLoader with BIDS root: {self.bids_root}")
            ...
            logger.error(f"Traceback: {traceback.format_exc()}")
            raise RuntimeError(f"StarterKitDataLoader initialization failed: {e}") from e
    ...
    def _validate_initialization_params(self) -> None:
        ...
        if not self.bids_root.exists():
            raise FileNotFoundError(f"BIDS root directory does not exist: {self.bids_root}")
    ...
    def _load_participants_data(self) -> None:
        ...
        with graceful_error_handler("loading participants data"):
            ...
            if missing_cols:
                error_msg = f"Missing required columns in participants.tsv: {missing_cols}"
                ...
                for col in missing_cols:
                    if col == "participant_id":
                        ...
                        logger.warning(f"Created missing {col} column with 'unknown' values")
    ...
    def _validate_participants_data(self) -> None:
        ...
        if duplicates.any():
            duplicate_ids = self.participants_data[duplicates]['participant_id'].tolist()
        ...
        if 'age' in self.participants_data.columns:
            ...
            if invalid_ages.any():
                logger.warning(f"Found {invalid_ages.sum()} participants with invalid ages")
        ...
        if 'sex' in self.participants_data.columns:
            ...
            if invalid_sex.any():
                logger.warning(f"Found {invalid_sex.sum()} participants with invalid sex values")
    ...
    def _optimize_participants_memory(self) -> None:
        ...
        for col in categorical_cols:
            if col in self.participants_data.columns:
                self.participants_data[col] = self.participants_data[col].astype('category')
        ...
        if 'age' in self.participants_data.columns:
            ...
            if age_series.max() <= 255 and age_series.min() >= 0:
                self.participants_data['age'] = age_series.astype('Int8')  # Nullable integer
    ...
    def _load_phenotype_data(self) -> None:
        ...
        with graceful_error_handler("loading phenotype data", default_return={}):
            ...
            logger.info(f"Loaded phenotype data for {len(self.phenotype_data)} instruments")
    ...
    def _load_cbcl_data(self, phenotype_dir: Path) -> None:
        ...
        try:
            ...
            if file_size_mb > self.memory_limit_mb * 0.2:
                logger.info(f"Large CBCL file ({file_size_mb:.1f}MB), using chunked loading")
                # Load in chunks to manage memory
                chunks = []
                chunk_size = max(1000, int(self.memory_limit_mb * 100))  # Adjust chunk size based on memory limit
            ...
            logger.info(f"Loaded CBCL data: {len(self.cbcl_df)} subjects ({memory_usage:.1f}MB)")
    ...
    def _validate_cbcl_data(self) -> None:
        ...
        expected_cbcl_cols = ['participant_id', 'p_factor', 'internalizing', 'externalizing', 'attention']
        ...
        if missing_cbcl_cols:
            ...
            for col in missing_cbcl_cols:
                if col != 'participant_id':  # participant_id is critical
                    ...
                    logger.warning(f"Created missing CBCL column '{col}' with NaN values")
        ...
        for col in numeric_cols:
            if col in self.cbcl_df.columns:
                ...
                if extreme_values.any():
                    logger.warning(f"Found {extreme_values.sum()} extreme values in CBCL column '{col}'")
        ...
        if 'participant_id' in self.cbcl_df.columns:
            ...
            if duplicates.any():
                logger.warning(f"Found {duplicates.sum()} duplicate subjects in CBCL data")
                self.cbcl_df = self.cbcl_df.drop_duplicates(subset=['participant_id'], keep='first')
        ...
        for col in numeric_cols:
            if col in self.cbcl_df.columns:
                ...
                try:
                    ...
                    if numeric_series.min() >= np.finfo(np.float32).min and numeric_series.max() <= np.finfo(np.float32).max:
    ...
    def _validate_phenotype_data(self) -> None:
        ...
        for instrument_name, df in self.phenotype_data.items():
            try:
                ...
                logger.debug(f"Instrument {instrument_name}: {len(df)} rows, {memory_usage:.1f}MB")
            ...
            if splits_file.exists():
                try:
                    ...
                    if self._validate_splits(self.official_splits):
                        ...
                        logger.warning("Official splits validation failed, regenerating...")
    ...
    def _validate_splits(self, splits: Dict[str, List[str]]) -> bool:
        ...
        try:
            ...
            if not set(splits.keys()) >= required_keys:
                logger.error(f"Missing required split keys. Expected: {required_keys}, Got: {set(splits.keys())}")
                return False

            # Check that splits are non-empty lists
            for split_name, split_data in splits.items():
                if not isinstance(split_data, list):
                    logger.error(f"Split '{split_name}' is not a list: {type(split_data)}")
                    return False
            ...
            for split_name, split_data in splits.items():
                ...
                if len(split_set) != len(split_data):
                    ...
                    logger.error(f"Split '{split_name}' contains {duplicates} duplicate subjects")
                    return False

                # Check for overlaps with other splits
                overlap = all_subjects.intersection(split_set)
                if overlap:
                    logger.error(f"Split '{split_name}' overlaps with previous splits: {overlap}")
                    return False
            ...
            if invalid_subjects:
                logger.warning(f"Found {len(invalid_subjects)} subjects with non-standard IDs")
            ...
            logger.info(f"Splits validation passed: {len(splits['train'])}/{len(splits['val'])}/{len(splits['test'])} subjects")
            return True
    ...
    def _create_subject_splits(
        ...
        if not (0.0 < train_ratio < 1.0):
            raise ValueError(f"Invalid train_ratio: {train_ratio}. Must be between 0 and 1.")
        ...
        if n_test < min_subjects_per_split:
            ...
            n_train = max(min_subjects_per_split, int(remaining * train_ratio / (train_ratio + val_ratio)))
            n_val = remaining - n_train

        logger.info(f"Creating splits with ratios: train={n_train/n_subjects:.2f}, val={n_val/n_subjects:.2f}, test={n_test/n_subjects:.2f}")
        ...
        if stratify_by and stratify_by in self.participants_data.columns:
            try:
                splits = self._create_stratified_splits(subjects, stratify_by, n_train, n_val, n_test)
            except Exception as e:
                logger.warning(f"Stratified splitting failed: {e}. Falling back to random splitting.")
    ...
    def _create_random_splits(self, subjects: List[str], n_train: int, n_val: int, n_test: int) -> Dict[str, List[str]]:
        ...
        logger.info(f"Created random splits: train={len(splits['train'])}, val={len(splits['val'])}, test={len(splits['test'])}")
        return splits
    ...
    def _create_stratified_splits(
        ...
        if not groups:
            raise ValueError(f"No valid groups found for stratification by {stratify_by}")
        ...
        for group_value, group_subjects in groups.items():
            ...
            splits["val"].extend(group_subjects[group_n_train:group_n_train + group_n_val])
            splits["test"].extend(group_subjects[group_n_train + group_n_val:group_n_train + group_n_val + group_n_test])

        logger.info(f"Created stratified splits by {stratify_by}: train={len(splits['train'])}, val={len(splits['val'])}, test={len(splits['test'])}")
        return splits
    ...
    def load_ccd_labels(self, split: str = "train") -> pd.DataFrame:
        ...
        with graceful_error_handler(f"loading CCD labels for {split}"):
            ...
            logger.info(f"Loading CCD labels for {len(split_subjects)} subjects in {split} split")
            ...
            if failed_subjects:
                logger.warning(f"Failed to load CCD data for {len(failed_subjects)} subjects: {failed_subjects[:5]}{'...' if len(failed_subjects) > 5 else ''}")

            if not ccd_data_list:
                raise pd.errors.EmptyDataError(f"No CCD data found for any subject in {split} split")
    ...
    def _process_ccd_events(self, events_df: pd.DataFrame) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if rt_column:
                ...
                valid_rt = (
                    (relevant_trials['response_time_target'] >= rt_min) &
            ...
            if accuracy_column:
                ...
                if accuracy_values.dtype == 'object':
                    ...
                    relevant_trials['success_target'] = accuracy_values.map(success_mapping)
                else:
                    # Numeric values - ensure 0/1 encoding
                    numeric_accuracy = pd.to_numeric(accuracy_values, errors='coerce')
                    relevant_trials['success_target'] = (numeric_accuracy > 0.5).astype(int)
                ...
                if invalid_success_count > 0:
                    logger.debug(f"Found {invalid_success_count} invalid success values")
            ...
            if 'onset' in events_df.columns:
                relevant_trials['onset_time'] = pd.to_numeric(events_df['onset'], errors='coerce')

            if 'duration' in events_df.columns:
                relevant_trials['trial_duration'] = pd.to_numeric(events_df['duration'], errors='coerce')
            ...
            output_columns = [
                'subject_id', 'session_id', 'task_name', 'onset_time',
                'response_time_target', 'success_target'
            ]

            # Only include columns that exist
            available_columns = [col for col in output_columns if col in valid_trials.columns]
    ...
    def _validate_and_clean_ccd_data(self, ccd_data: pd.DataFrame) -> pd.DataFrame:
        ...
        if invalid_subjects.any():
            logger.warning(f"Removing {invalid_subjects.sum()} trials with invalid subject IDs")
    ...
    def load_cbcl_labels(self, split: str = "train") -> pd.DataFrame:
        ...
        with graceful_error_handler(f"loading CBCL labels for {split}"):
            ...
            if not hasattr(self, 'cbcl_df') or self.cbcl_df is None or self.cbcl_df.empty:
                raise pd.errors.EmptyDataError("No CBCL data available. Check phenotype data loading.")
            ...
            logger.info(f"Loading CBCL labels for {len(split_subjects)} subjects in {split} split")
            ...
            if cbcl_split_data.empty:
                raise pd.errors.EmptyDataError(f"No CBCL data found for subjects in {split} split")
    ...
    def _process_cbcl_data(self, cbcl_data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            required_cbcl_columns = ['p_factor', 'internalizing', 'externalizing', 'attention']
            ...
            if 'attention' in processed_data.columns:
                ...
                logger.warning("No attention scores available for binary classification")
            ...
            for col in required_cbcl_columns + ['binary_label']:
                if col in processed_data.columns:
                    ...
                    completeness_stats[col] = valid_count / total_count if total_count > 0 else 0

            logger.info(f"CBCL data completeness: {completeness_stats}")

            # Select final columns
            output_columns = ['participant_id'] + required_cbcl_columns + ['binary_label']
            available_output_columns = [col for col in output_columns if col in processed_data.columns]

            result_df = processed_data[available_output_columns].copy()

            # Add metadata
            result_df['cbcl_complete'] = result_df[required_cbcl_columns].notna().all(axis=1)
            result_df['n_valid_cbcl'] = result_df[required_cbcl_columns].notna().sum(axis=1)

            logger.debug(f"Processed CBCL data: {len(result_df)} subjects with {result_df['cbcl_complete'].sum()} complete profiles")
            ...
            minimal_df = pd.DataFrame({'participant_id': cbcl_data.get('participant_id', [])})
    ...
    def compute_official_metrics(self, split: str = "val") -> Dict[str, float]:
        ...
        with graceful_error_handler(f"computing official metrics for {split}"):
            ...
            try:
                quality_metrics = self._compute_data_quality_metrics(ccd_data, cbcl_data)
        ...
        if 'response_time_target' in ccd_data.columns:
            ...
            if len(rt_series) > 0:
                ...
                for p in percentiles:
                    metrics[f'ccd_response_time_p{p}'] = float(np.percentile(rt_series, p))
            else:
                metrics.update(self._get_default_ccd_metrics())
        ...
        if 'success_target' in ccd_data.columns:
            ...
            if len(success_series) > 0:
                ...
                if len(success_series) > 10:
                    ...
                    metrics['ccd_success_rate_ci_lower'] = float(max(0, center - margin))
                    metrics['ccd_success_rate_ci_upper'] = float(min(1, center + margin))
    ...
    def _compute_cbcl_metrics(self, cbcl_data: pd.DataFrame) -> Dict[str, float]:
        ...
        # Data completeness
        complete_profiles = cbcl_data['cbcl_complete'].sum() if 'cbcl_complete' in cbcl_data.columns else 0
        metrics['cbcl_completeness_rate'] = float(complete_profiles / len(cbcl_data)) if len(cbcl_data) > 0 else 0.0

        return metrics

    def _compute_data_quality_metrics(self, ccd_data: pd.DataFrame, cbcl_data: pd.DataFrame) -> Dict[str, float]:
        ...
        if not ccd_data.empty and not cbcl_data.empty:
            ...
            metrics['subject_overlap_rate'] = float(overlap / union) if union > 0 else 0.0

        # Data quality flags
        if not ccd_data.empty:
            if 'complete_trial' in ccd_data.columns:
                metrics['ccd_complete_trials_rate'] = float(ccd_data['complete_trial'].mean())
    ...
    def _get_default_metrics(self) -> Dict[str, float]:
        ...
        return {
            **self._get_default_ccd_metrics(),
            **self._get_default_cbcl_metrics()
        }
        ...
        with graceful_error_handler("generating data summary"):
            ...
            if hasattr(self, 'participants_df') and self.participants_df is not None:
                summary['participants'] = {
                    ...
                    'age_stats': self._get_age_statistics() if 'age' in self.participants_df.columns else None
                }
            else:
            ...
            if hasattr(self, 'phenotype_df') and self.phenotype_df is not None:
                summary['phenotype'] = {
                    ...
                    'memory_usage_mb': float(self.phenotype_df.memory_usage(deep=True).sum() / 1024 / 1024)
                }
            else:
            ...
            if hasattr(self, 'official_splits') and self.official_splits is not None:
                summary['splits'] = {
                    split: len(subjects) for split, subjects in self.official_splits.items()
                }
            ...
            if self.enable_caching:
                summary['cache'] = {
                    ...
                    'cached_splits': list(self.ccd_cache.keys()) + list(self.cbcl_cache.keys())
                }
            ...
            try:
                ...
                summary['memory_usage'] = {
                    ...
                    'utilization_pct': (current_memory / (self.memory_limit_gb * 1024)) * 100
                }
            except Exception as e:
    ...
    def _get_cbcl_completeness_stats(self) -> Dict[str, float]:
        ...
        for col in available_columns:
            ...
            completeness[f'{col}_completeness'] = float(valid_count / total_count) if total_count > 0 else 0.0

        # Overall completeness (all columns valid)
        if available_columns:
            all_valid = self.cbcl_df[available_columns].notna().all(axis=1).sum()
            completeness['overall_completeness'] = float(all_valid / len(self.cbcl_df)) if len(self.cbcl_df) > 0 else 0.0
    ...
    def get_ccd_labels(self, subject_id: str, session_id: str = "001") -> Dict[str, Any]:
        """
        ...
        ccd_files = list(self.bids_root.glob(f"{subject_key}/ses-{session_id}/eeg/*_task-CCD_events.tsv"))
            ...
            if "response_time" in events_df.columns:
                ...
                if len(rt_data) > 0:
                    labels.update({
                        ...
                        "response_time": rt_data.iloc[-1],  # Last RT for single prediction
                ...
                if len(success_data) > 0:
                    labels.update({
                        ...
                        "success": int(success_data.iloc[-1])  # Last success for single prediction
                    })
    ...
    def get_subject_metadata(self, subject_id: str) -> Dict[str, Any]:
        ...
        subject_data = self.participants_df[self.participants_df["participant_id"] == subject_key]
...
class OfficialMetrics:
    ...
    @staticmethod
    def compute_challenge1_metrics(predictions: Dict[str, np.ndarray], targets: Dict[str, np.ndarray]) -> Dict[str, float]:
        """
        ...
        if "response_time" in predictions and "response_time" in targets:
            ...
            if valid_mask.sum() > 0:
                ...
                # RMSE
                metrics["rt_rmse"] = np.sqrt(mean_squared_error(rt_true_valid, rt_pred_valid))
        ...
        if "success" in predictions and "success" in targets:
            ...
            if success_pred.max() <= 1.0 and success_pred.min() >= 0.0:
                ...
                try:
                    ...
                    metrics["success_auprc"] = average_precision_score(success_true, success_pred)
            ...
            metrics["success_balanced_acc"] = balanced_accuracy_score(success_true, success_pred_binary)

        return metrics

    @staticmethod
    def compute_challenge2_metrics(predictions: Dict[str, np.ndarray], targets: Dict[str, np.ndarray]) -> Dict[str, float]:
        """
        ...
        if "binary_label" in predictions and "binary_label" in targets:
            ...
            if binary_pred.max() <= 1.0 and binary_pred.min() >= 0.0:
                try:
                    ...
                    metrics["binary_auprc"] = average_precision_score(binary_true, binary_pred)
                except ValueError as e:
            ...
            metrics["binary_balanced_acc"] = balanced_accuracy_score(binary_true, binary_pred_int)
...
class SubmissionValidator:
    ...
    def __init__(self):
        self.challenge1_schema = {
            "required_columns": ["subject_id", "session_id", "response_time_pred", "success_pred"],
            "dtypes": {
                "subject_id": "object",
                "session_id": "object",
        ...
        self.challenge2_schema = {
            "required_columns": ["subject_id", "p_factor_pred", "internalizing_pred",
                               "externalizing_pred", "attention_pred", "binary_pred"],
            "dtypes": {
                "subject_id": "object",
    ...
    def validate_challenge1_submission(self, submission_df: pd.DataFrame) -> Dict[str, Any]:
        """Validate Challenge 1 submission format."""
        return self._validate_submission(submission_df, self.challenge1_schema, "Challenge 1")

    def validate_challenge2_submission(self, submission_df: pd.DataFrame) -> Dict[str, Any]:
        """Validate Challenge 2 submission format."""
        return self._validate_submission(submission_df, self.challenge2_schema, "Challenge 2")

    def _validate_submission(self, df: pd.DataFrame, schema: Dict, challenge_name: str) -> Dict[str, Any]:
        ...
        for col, expected_dtype in schema["dtypes"].items():
            if col in df.columns:
                try:
                    ...
                    results["warnings"].append(f"Column {col} cannot be converted to {expected_dtype}")
        ...
        if challenge_name == "Challenge 1":
            ...
            if "success_pred" in df.columns:
                if df["success_pred"].min() < 0 or df["success_pred"].max() > 1:
                    results["warnings"].append("success_pred values should be probabilities [0, 1]")
            ...
            if "binary_pred" in df.columns:
                if df["binary_pred"].min() < 0 or df["binary_pred"].max() > 1:
                    results["warnings"].append("binary_pred values should be probabilities [0, 1]")
