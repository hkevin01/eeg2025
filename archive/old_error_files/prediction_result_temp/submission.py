...
class Submission:
    ...
    def __init__(self):
        ...
        # Challenge 2: Externalizing with CompactCNN
    ...
    def predict_externalizing(self, eeg_data):
        ...
        Args:
            eeg_data: (batch_size, n_channels=64, n_samples=2000)
                     RestingState EEG data

        Returns:
            predictions: (batch_size, 6) externalizing features
        """
        with torch.no_grad():
            eeg_tensor = torch.FloatTensor(eeg_data).to(self.device)
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Challenge 2: 64 channels, 2000 samples (RestingState)
        dummy_eeg_c2 = torch.randn(batch_size, 64, 2000).numpy()
        print(f"   Challenge 2 input shape: {dummy_eeg_c2.shape}")
