...
class TestOfficialLabelsAndSplits:
    ...
    def mock_bids_root(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            participants_data = pd.DataFrame({
                'participant_id': ['sub-001', 'sub-002', 'sub-003', 'sub-004', 'sub-005'],
                'age': [10, 11, 12, 13, 14],
                'sex': ['M', 'F', 'M', 'F', 'M']
            })
            participants_data.to_csv(bids_root / 'participants.tsv', sep='\t', index=False)
            ...
            for sub_id in participants_data['participant_id']:
                ...
                if 'CCD' in str(eeg_file):
                    events_file = sub_dir.parent / 'func' / f'{sub_id}_ses-001_task-CCD_events.tsv'
    ...
    def test_split_isolation(self, mock_bids_root, mock_splits):
        ...
        # Check no overlaps
        assert len(train_subjects & val_subjects) == 0, "Train-validation overlap detected"
        assert len(train_subjects & test_subjects) == 0, "Train-test overlap detected"
        assert len(val_subjects & test_subjects) == 0, "Validation-test overlap detected"
        ...
        for split_name, split_subjects in mock_splits.items():
            ...
            for subject in split_subjects:
                ...
                assert subject not in (
                    set(mock_splits['train']) | set(mock_splits['val']) | set(mock_splits['test'])
                ) - set(split_subjects)
...
class TestLeakageControls:
    ...
    def mock_eeg_data(self):
        ...
        for i, split in enumerate(['train', 'val', 'test']):
            ...
            eeg_data = np.random.randn(64, 1000) * scale + baseline  # 64 channels, 1000 timepoints
    ...
    def mock_session_info(self):
        ...
        return {
            'sub-001_ses-001_RS': {'split': 'train', 'subject_id': 'sub-001', 'session_id': 'ses-001'},
            'sub-002_ses-001_RS': {'split': 'val', 'subject_id': 'sub-002', 'session_id': 'ses-001'},
            'sub-003_ses-001_RS': {'split': 'test', 'subject_id': 'sub-003', 'session_id': 'ses-001'}
        }
...
class TestDataIntegration:
    ...
    def test_hbn_dataset_with_splits(self, mock_bids_root=None, mock_splits=None):
        ...
        with patch('dataio.bids_loader.read_raw_bids'), \
