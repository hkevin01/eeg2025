...
class TestNormalizationLeakage:
    ...
    def test_normalization_train_only_fitting(self, mock_multi_session_data):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            train_sessions = {
                k: v for k, v in data.items() if session_info[k]["split"] == "train"
            }
    ...
    def test_val_test_normalization_uses_train_stats(self, mock_multi_session_data):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            val_session_id = [
                k for k in data.keys() if session_info[k]["split"] == "val"
            ][0]
            ...
            assert (
                abs(val_mean) > 5
            )  # Should be shifted significantly from train distribution
    ...
    def test_leakage_detection_mixed_splits(self, mock_multi_session_data):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            mixed_sessions = {}
            mixed_sessions.update(
                {k: v for k, v in data.items() if session_info[k]["split"] == "train"}
            )
            mixed_sessions.update(
                {k: v for k, v in data.items() if session_info[k]["split"] == "val"}
            )
    ...
    def test_session_aware_sampling_groups_correctly(self):
        ...
        for batch in batches:
            if len(batch) > 1:
                ...
                sessions = [
                    dataset.get_session_info(idx)["session_id"] for idx in batch
                ]
...
class TestValidationReproducibility:
    ...
    def test_split_generation_reproducibility(self):
        ...
        splits3 = self._generate_stratified_splits(
            participants, random_seed=999
        )  # Different seed
        ...
        assert (
            splits1["train"] != splits3["train"]
            or splits1["val"] != splits3["val"]
            or splits1["test"] != splits3["test"]
        )
    ...
    def _generate_stratified_splits(self, participants_df, random_seed=42):
        ...
        participants_df["age_bin"] = pd.cut(
            participants_df["age"], bins=3, labels=[0, 1, 2]
        )
        participants_df["strata"] = (
            participants_df["sex"].astype(str)
            + "_"
    ...
    def test_normalization_determinism(self):
        ...
        np.testing.assert_array_almost_equal(
            normalized1, normalized2, err_msg="Normalization not deterministic"
        )
