...
def test_challenge_components():
    ...
    try:
        ...
        starter_kit_path = Path(
            "/home/kevin/Projects/eeg2025/src/dataio/starter_kit.py"
        )
        ...
        total_components = 4
        total_found = (
            min(splits_found, len(splits_features))
            + min(prep_found, len(prep_features))
            + min(enhanced_found, len(enhanced_features))
        )
        total_possible = (
            len(splits_features) + len(prep_features) + len(enhanced_features)
        )
        ...
        logger.info(
            f"✅ Found {len(found_patterns)}/{len(error_patterns)} error handling patterns"
        )
        ...
        logger.info(
            f"✅ Found {len(found_memory)}/{len(memory_patterns)} memory management features"
        )
        ...
        logger.info(
            f"✅ Found {len(found_validation)}/{len(validation_patterns)} validation features"
        )
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            expected_files = [
                ...
                / "ses-001"
                / "func"
                / "sub-001_ses-001_task-CCD_events.tsv",
            ]
            ...
            missing_cols = set(required_participants_cols) - set(
...
def test_python_syntax():
    ...
    try:
        ...
        if indentation_errors:
            logger.warning(
                f"⚠️ Found {len(indentation_errors)} potential indentation issues"
            )
...
def main():
    ...
    try:
        ...
        if passed_tests == total_tests:
            ...
            logger.info(
                "✅ Official splits, leakage protection, and challenge compliance implemented"
            )
