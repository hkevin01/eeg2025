...
print("=" * 80)
print("🚀 FULL DATASET FOUNDATION TRAINING")
print("=" * 80)

# Full config - larger model, full dataset
CONFIG = {
    'hidden_dim': 128,      # Larger model for better features
    ...
    'learning_rate': 1e-3,
    'max_samples': None,    # Use ALL 38,506 samples
}
...
def main():
    ...
    if CONFIG['max_samples'] is not None and len(full_dataset) > CONFIG['max_samples']:
        ...
        print(f"✅ Using ALL {len(dataset)} samples (full dataset)")
    ...
    for epoch in range(CONFIG['epochs']):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({
                ...
                'val_loss': val_loss,
                'config': CONFIG
            }, checkpoint_dir / "full_best.pth")
            print(f"  💾 Saved checkpoint (best val loss: {val_loss:.4f})")

    # Save history
    log_dir = Path(__file__).parent.parent / "logs"
    log_dir.mkdir(exist_ok=True)
    with open(log_dir / "full_history.json", 'w') as f:
    ...
    print("Checkpoint: checkpoints/full_best.pth")
    print("History: logs/full_history.json")
    print("\n💡 Next: Retrain Challenge 1 & 2 with new foundation model:")
