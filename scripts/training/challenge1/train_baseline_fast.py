"""
Train baseline CNN model with pre-computed features for fast training.
This establishes the baseline performance to compare against hybrid model.
"""
...
class FastHDF5Dataset(Dataset):
    ...
    def __init__(self, hdf5_paths, use_features=False):
    ...
    def __getitem__(self, idx):
        ...
        with h5py.File(hdf5_path, 'r') as f:
            ...
            # Load pre-computed features if needed
            if self.use_features and 'neuro_features' in f:
...
def train_epoch(model, train_loader, criterion, optimizer, device, use_features=False):
    model.train()
    total_loss = 0
...
def validate(model, val_loader, criterion, device, use_features=False):
    model.eval()
    total_loss = 0
    predictions = []
    targets = []
...
def main():
    print("="*80)
    print("🎯 BASELINE CNN TRAINING (Fast with Pre-computed Features)")
    print("="*80)
    ...
    full_dataset = FastHDF5Dataset(hdf5_files, use_features=False)
    ...
    # Model (baseline - no features)
    model = HybridNeuroModel(
        input_channels=129,
        use_neuro_features=False,  # BASELINE: no features
        dropout_rate=0.4
    ).to(device)
    
    n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"\nModel parameters: {n_params:,}")
    print("Mode: Baseline CNN (no neuroscience features)")
    ...
    for epoch in range(n_epochs):
        ...
        # Train
        train_loss = train_epoch(model, train_loader, criterion, optimizer, device, use_features=False)
        
        # Validate
        val_loss, val_nrmse = validate(model, val_loader, criterion, device, use_features=False)
        ...
        if val_nrmse < best_nrmse:
            ...
            torch.save(checkpoint, PROJECT_ROOT / 'checkpoints' / 'baseline_best.pth')
    ...
    print(f"📁 Model saved to: checkpoints/baseline_best.pth")
